layui.use(['form','layer','jquery','upload'],function(){
    ...
    form.on('submit(btnSubmitScenicSpotBinding)', function(data){
        //window.resource("/system/scenicSpotBinding/editScenicSpotBinding", data.field, function (data) {}, false,"POST",'scenicSpotBindingListTable');
        var formData = new FormData($("#userForm")[0]);
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/scenicSpotBinding/editScenicSpotBinding",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
