layui.use(['table','form','layer','jquery'],function(){
	...
	setTimeout(function(){
        ...
        url : '/system/treasureHunt/getBroadcastContentHuntList',
        cellMinWidth : 100,
        page : true,
        height : "full-125",
        request: {
        ...
        cols : [[
            ...
            {field: 'broadcastContent', title: '寻宝播报内容', minWidth:100, align:"center"},
    ...
    table.on('tool(broadcastContentList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            window.resources("SYSTEM_BROADCAST_DELETE", function (e) {
                if (e.state == "200"){
                    window.resourcedel("/system/treasureHunt/delBroadcastHuntContentExtend", {broadcastResId : data.broadcastResId}, function (data) {}, false,"GET","broadcastContentListTable");
                }else if (e.state == "400"){
                    layer.msg(e.msg, {icon: 5,time: 1000,shift: 6});
                }
            }, false,"GET");
    ...
    function openAddBroadcastContentExtend(broadcastId) {
        layer.open({
            ...
            offset: '10%',
            area: ['800px', '380px'],
            content: '/page/system/treasureHunt/html/broadcastContentAdd.html',
    ...
    function openEditCorpusExtend(edit){
        ...
        setTimeout(function(){
            layer.open({
                ...
                offset: '10%',
                area: ['800px', '380px'],
                content: '/page/system/treasureHunt/html/broadcastContentEdit.html',
                success : function(layero, index){
                    var body = layui.layer.getChildFrame('body', index);
                    if(edit){
                        top.layer.close(dex);
