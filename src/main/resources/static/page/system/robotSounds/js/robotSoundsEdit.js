layui.use(['form','layer','jquery','upload'],function(){
    ...
    var soundsMediaType = $(".soundsMediaType").val();
    if (soundsMediaType == 1) {
        $("#imageForm").css("display","block");
        $("#videoForm").css("display","none");
        $("#audioForm").css("display","none");
        $("input:radio[name='type'][value="+soundsMediaType+"]").prop('checked','true');
        form.render(); //更新全部
    }else if (soundsMediaType == 3) {
        $("#imageForm").css("display","none");
        $("#videoForm").css("display","block");
        $("#audioForm").css("display","none");
        $("input:radio[name='type'][value="+soundsMediaType+"]").prop('checked','true');
        form.render(); //更新全部
    }else if (soundsMediaType == 2) {
        $("#imageForm").css("display","none");
        $("#videoForm").css("display","none");
    ...
    form.on('submit(btnEditImageForm)', function(data){
        ...
        $.ajax({
            url: "/system/robotSounds/editRobotSoundsWrittenWords",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
            ...
            success: function (e) {
                if (e.state == "200") {
                    setTimeout(function(){
                        ...
                        parent.layui.table.reload('robotSoundsListTable');
                    },500);
                } else {
                    setTimeout(function(){
                        top.layer.close(index);
    ...
    form.on('submit(btnEditVideoForm)', function(data){
        ...
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/robotSounds/editRobotSoundsVideo",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
    ...
    //监听提交音频
    form.on('submit(btnEditAudioForm)', function(data){
        var formData = new FormData($("#audioForm")[0]);
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/robotSounds/editRobotSoundsAudio",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
