layui.use(['form','layer','jquery','upload'],function(){
    ...
    form.on('submit(btnSubmitPayBack)', function(data){
        ...
        var index = top.layer.msg('储值订单退款中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/order/doPayStoredValueOrderBackDeposit",
            data: {
            ...
            success: function (e) {
                if (e.state == "200") {
                    setTimeout(function(){
                        ...
                        parent.layui.table.reload('storedValueOrderListTable');
                    },500);
                } else {
                    setTimeout(function(){
                        top.layer.close(index);
