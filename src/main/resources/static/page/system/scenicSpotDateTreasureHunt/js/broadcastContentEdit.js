layui.use(['form','layer','jquery','upload'],function(){
    ...
    if (mediaType == 1) {
        ...
        $("input:radio[name='type'][value="+mediaType+"]").prop('checked','true');
        form.render(); //更新全部
    }else if (mediaType == 2) {
    ...
    form.on('submit(btnAddImageForm)', function(data){
        ...
        $.ajax({
            url: "/system/treasureHunt/editBroadcastHuntContentExtendImage",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
            ...
            success: function (e) {
                if (e.state == "200") {
                    setTimeout(function(){
                        ...
                        parent.layui.table.reload('broadcastContentListTable');
                    },500);
                } else {
                    setTimeout(function(){
                        top.layer.close(index);
    ...
    form.on('submit(btnAddVideoForm)', function(data){
        ...
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/treasureHunt/editBroadcastHuntContentExtendVideo",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
    ...
    form.on('submit(btnAddAudioForm)', function(data){
        ...
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/treasureHunt/editBroadcastHuntContentExtendAudio",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
