layui.use(['form','layer','jquery','upload'],function(){
    ...
    if (randomType == 1) {
        ...
        $("input:radio[name='type'][value="+randomType+"]").prop('checked','true');
        form.render(); //更新全部
    ...
    form.on('submit(btnEditImageForm)', function(data){
        ...
        $.ajax({
            url: "/system/randomProblem/editRandomProblemImage",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
    ...
    form.on('submit(btnEditVideoForm)', function(data){
        ...
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/randomProblem/editRandomProblemVideo",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
    ...
    form.on('submit(btnAddAudioForm)', function(data){
        ...
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        $.ajax({
            url: "/system/randomProblem/editRandomProblemAudio",
            data: formData,
            type: "POST",
            async: true,
            cache:false,
