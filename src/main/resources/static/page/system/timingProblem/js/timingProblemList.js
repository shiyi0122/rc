layui.use(['form','layer','table'],function(){
    ...
    var tableIns = table.render({
        ...
        url : '/system/timingProblem/getTimingProblemList',
        cellMinWidth : 100,
        page : true,
        height : "full-125",
        request: {
        ...
        cols : [[
            ...
            {field: 'timingProblem', title: '播报标题', minWidth:100, align:"center"},
            {field: 'timingAnswers', title: '播报内容', minWidth:100, align:"center"},
            {field: 'timingType', title: '播报类型', align:'center',templet:function(d){
                    if(d.timingType == "1"){
                        return "文字类型";
                    }else if(d.timingType == "2"){
                        return "视频类型";
                    }else if(d.timingType == "3"){
                        return "音频类型";
                    }
                }},
            {field: 'timingResRul', title: '音视频资源地址', minWidth:100, align:"center"},
            {field: 'timingResRulPic', title: '图片资源地址', minWidth:100, align:"center"},
            {field: 'weight', title: '权重', minWidth:100, align:"center"},
            {field: 'timingState', title: '播报状态', align:'center',templet:function(d){
                    if(d.timingState == "1"){
                        return "启用";
                    }else if(d.timingState == "0"){
                        return "禁用";
                    }
                }},
    ...
    $("#btnSearch").on("click",function(){
        if($(".randomProblemVal").val() != ''){
            table.reload("randomProblemListTable",{
                ...
                where: {
                    timingProblem: $(".randomProblemVal").val()  //搜索的关键字
                }
            })
        }else{
    ...
    table.on('tool(randomProblemList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            window.resources("RESOURCES_RANDOM_PROBLEM_DELETE", function (e) {
                if (e.state == "200"){
                    window.resourcedel("/system/timingProblem/delTimingProblem", {problemId : data.problemId}, function (data) {}, false,"GET","randomProblemListTable");
                }else if (e.state == "400"){
                    layer.msg(e.msg, {icon: 5,time: 1000,shift: 6});
                }
            }, false,"GET");
            ...
            window.resources("RESOURCES_RANDOM_PROBLEM_EDITVALID", function (e) {
                if (e.state == "200") {
                    layer.confirm('确定修改为禁用吗？',{icon:3, title:'提示信息'},function(index){
                        ...
                        $.ajax({
                            url: "/system/timingProblem/editTimingProblemValid",
                            data: {
    ...
    function openAddRandomProblem() {
        layer.open({
            ...
            offset: '10%',
            area: ['800px', '500px'], //宽高
            content: '/page/system/timingProblem/html/timingProblemAdd.html'
        });
    };
    ...
    function openEditRandomProblem(edit){
        ...
        setTimeout(function(){
            layer.open({
                ...
                offset: '10%',
                area: ['800px', '430px'], //宽高
                content: '/page/system/timingProblem/html/timingProblemEdit.html',
                success : function(layero, index){
                    var body = layui.layer.getChildFrame('body', index);
                    if(edit){
                        top.layer.close(dex);
                        body.find(".problemId").val(edit.problemId);
                        body.find(".timingProblem").val(edit.timingProblem);
                        body.find(".timingAnswers").val(edit.timingAnswers);
                        body.find(".timingType").val(edit.timingType);
