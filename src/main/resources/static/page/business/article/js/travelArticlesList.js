layui.use(['form','layer','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {title: '操作', minWidth:175, templet:'#travelArticlesListBar',fixed:"right",align:"center"}
        ]]
    });

    //列表操作
    table.on('tool(travelArticlesList)', function(obj){
        var layEvent = obj.event,
            data = obj.data;

        if(layEvent === 'edit'){ //编辑
            window.resources("BUSINESS_TRAVEL_ARTICLES_UPDATE", function (e) {
                if (e.state == "200") {
            ...
            window.resources("BUSINESS_TRAVEL_ARTICLES_DELETE", function (e) {
                if (e.state == "200"){
                    window.resourcedel("/business/article/delArticle", {id : data.id}, function (data) {}, false,"GET","travelArticlesListTable");
                }else if (e.state == "400"){
                    layer.msg(e.msg, {icon: 5,time: 1000,shift: 6});
                }
            }, false,"GET");
    ...
    function openEditTravelArticles(edit){
        ...
        setTimeout(function(){
            layer.open({
                ...
                // offset: '10%',
                area: ['100%', '100%'],
                content: '/page/business/article/html/travelArticlesEdit.html',
